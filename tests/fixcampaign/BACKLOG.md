...
- **Program-scope residual — DONE + MERGED (Session 63, fix `625604fe`,
  merge `f35f3f5b`).**
  S53 deferred program-scope blocks ("keep the raw path"); that left
  `#if DFx / #elif DFy` chains wrapping whole `function_definition`s untyped
  (mslfR2 "cubes": `out float ogd`, `p1 *= g_rot`, `10.0*vec2(...)` all raw).
  Fix (`ast_transformer.py`): drop the `not self._global_scope` gate on the
  S53 router; add `function_definition` to `_PREPROC_ROUTABLE_STMTS`; abort
  the block to raw text when it contains an ENTRY-POINT def (S59 owns that).
  Out-param call sites resolve for free (source-order `function_signatures`).
  Blast radius 35 PASS shaders re-tested → **0 regressed**; cubes.glsl compiles;
  corpus shader MdVcRK improved `[G,K,T]→[G]` (residual is independent G). Unit
  +3 (`test_transformer_preproc_matrix.py` Part 3). All four gates green.
